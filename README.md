...
**[plans/day2_plan.md](plans/day2_plan.md)** — 내일(8/20) 아침부터 밤까지 순서대로 할 일.
큐 확인 → 결과 기록 → Gate 1 판정 → 오후 실차 테스트 → 본선 모델 2개 확정.
...
```
Final = Lap Time + (Off-Track 횟수 × 1.0s)     ※ 리셋 중 시계 정지
```

운영진 확인 결과 **리셋 중 시계가 멈춘다.** 탈선 실비용은 페널티 1초 + 재가속 약 1초 =
**약 2초**뿐이다. 이건 속도를 한 단계 올려 얻는 이득보다 작다.
...
1. **1차는 저금, 2차는 전력** — 1·2차 사이 모델·스로틀 교체가 허용된다. 1차에서 유효 기록을
   확보하면 2차는 잃을 것이 없는 공짜 옵션이 된다.
...
| 항목 | 값 |
| :--- | :--- |
| 트랙 | **re:Invent 2018** (내부명 `reinvent_base`, 17.6m × 76cm) — 단 A to Z Speedway(`reInvent2019_wide`, 16.64m × 107cm) 가능성 미배제 |
| 트랙 방향 | **반시계만 지원** (Clockwise 비활성) → 팀 전체 자동 동일 |
